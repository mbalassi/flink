...
class RowTypeInfo(
  ...
  extends CaseClassTypeInfo[Row](classOf[Row], Array(), fieldTypes, fieldNames) {   //TypeInformation for Case Classes

  def this(fields: Seq[Expression]) = this(fields.map(_.typeInfo), fields.map(_.name)) // Expression (children, name, typeinfo)
...
object  RowTypeInfo extends App {

  val rowTypeInfo = new RowTypeInfo(Seq(BasicTypeInfo.INT_TYPE_INFO,BasicTypeInfo.INT_TYPE_INFO), Seq("field1", "field2"))
    println(rowTypeInfo.getFieldNames.mkString(","))
    println(rowTypeInfo.getTypeAt(1))
