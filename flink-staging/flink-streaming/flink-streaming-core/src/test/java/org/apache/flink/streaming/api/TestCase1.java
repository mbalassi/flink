...
public class TestCase1 {
	 ...
	 * 
	 * @param args
	 *            0:maxValue for data source 1:window size 2:slide size 3:version
	 * @throws Exception
	 */
	public static void main(String[] args) throws Exception {
		...
		// long windowSize = Long.parseLong(args[1]);
		long windowSize = 50;

		// long slideSize = Long.parseLong(args[2]);
		long slideSize = 10;
		...
		case 1:
			 ...
			 */
			testDataStream = testDataStream.window(Time.of(windowSize, timestamp)).every(Time.of(slideSize, timestamp)).max(0)
					.flatten();
			break;
		case 2:
			...
			triggers.add(new TimeTriggerPolicy<Long>(slideSize, new TimestampWrapper<Long>(timestamp, 0)));
			evictors.add(new TimeEvictionPolicy<Long>(windowSize, new TimestampWrapper<Long>(timestamp, 0)));
			
			testDataStream = testDataStream.transform(
					"WindowInv",
					testDataStream.getType(),
		...
		case 3:
			...
			testDataStream = testDataStream.transform(
					"WindowInv",
					testDataStream.getType(),
					new WindowReduceInvokable<Long>(maxLong,windowSize,slideSize,new TimestampWrapper<Long>(timestamp, 0)));
